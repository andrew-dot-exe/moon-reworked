...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const updateCellData = (x: number, z: number) => {
      return map.updateCellData(x, z);
    };
    ...
    onMounted(() => {
      ...
      hdrLoader.load(
        '/render-background.hdr', // Путь к HDRI
        (texture) => {
          // Устанавливаем HDRI как фон сцены
          texture.mapping = THREE.EquirectangularReflectionMapping;
          scene.background = texture;
          scene.environment = texture; // (опционально) для отражений
        },
        undefined,
        (error) => {
