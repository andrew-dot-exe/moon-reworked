<template>
    <div class="master-screen-acount">
      ...
      <form class="main-container" @submit.prevent="handleLogin">
        <div class="create-account">
          <div class="titile-block">Регистрация</div>
          ...
          <div class="account-information">
            <div class="div">
              ...
              <div class="value">
                <input type="text" id="nik" v-model="name" required class="text-wrapper-2" />
              </div>
            </div>
        ...
        <button class="button-sign-in">
          <div class="text-wrapper-3">Зарегистрироваться</div>
        </button>
      </form>
    </div>
  </template>
  ...
  const handleLogin = async () => {
    if (name.value && email.value && password.value) {
      const response: Promise<number> = userApi.register(name.value, email.value, password.value);
